...
exports.checkBingoWin = exports.generateBingoCard = void 0;
...
const checkBingoWin = (card, drawnNumbers) => {
    // Check rows
    for (let row = 0; row < 5; row++) {
    ...
    // Check columns
    for (let col = 0; col < 5; col++) {
        if (card.every((row) => row[col] === 0 || drawnNumbers.has(row[col])))
            return true;
    }
    // Check diagonals
    if (card.every((row, i) => row[i] === 0 || drawnNumbers.has(row[i])) ||
